...
import { logApi, logger } from '@/lib/logger';
...
const fetchDashboardStats = async (userId: string) => {
  try {
    logApi('debug', 'Fetching dashboard stats', '/api/direct/dashboard-stats/user/{userId}', { userId });
    ...
    logApi('debug', 'Dashboard stats response received', '/api/direct/dashboard-stats/user/{userId}', {
      userId,
    ...
    if (statsResponse.ok) {
      ...
      logApi('info', 'Dashboard stats received successfully', '/api/direct/dashboard-stats/user/{userId}', {
        userId,
      ...
      try {
        ...
        if (prsResponse.ok) {
          ...
          logApi('info', 'PRS scores received successfully', '/api/direct/prs/user/{userId}', {
            userId,
            totalRecords: prsData.length
          });
        ...
        logApi('warn', 'Failed to fetch PRS data, using stats only', '/api/direct/prs/user/{userId}', {
          userId,
      ...
      logApi('info', 'No dashboard data found for user', '/api/direct/dashboard-stats/user/{userId}', { userId });
      return { prsScores: [], stats: null, hasData: false, totalScores: 0, averageScore: 0, diseasesAnalyzed: 0 };
    } else {
      logApi('warn', 'Dashboard Stats API error', '/api/direct/dashboard-stats/user/{userId}', {
        userId,
        status: statsResponse.status,
    ...
    logApi('warn', 'Backend not available, using fallback data', 'dashboard_stats', {
      userId,
      error: error.message
    });
...
export default function DashboardPage() {
  ...
  const handleUploadSuccess = async (data: any) => {
    logApi('info', 'File uploaded successfully', 'file_upload', { userId, fileData: data });
    ...
    setTimeout(async () => {
      try {
        logApi('debug', 'Starting data refresh after upload', 'data_refresh', { userId });
        ...
        try {
          ...
          logApi('debug', 'PRS scores query invalidated', 'query_invalidation', { userId });
        } catch (error) {
          logApi('warn', 'Failed to invalidate PRS scores query', 'query_invalidation', {
            userId,
            error: error.message
          });
        }
        ...
        try {
          ...
          logApi('debug', 'Recommendations query invalidated', 'query_invalidation', { userId });
        } catch (error) {
          logApi('warn', 'Failed to invalidate recommendations query', 'query_invalidation', {
            userId,
            error: error.message
          });
        }
        ...
        try {
          ...
          logApi('debug', 'Dashboard stats query invalidated', 'query_invalidation', { userId });
        } catch (error) {
          logApi('warn', 'Failed to invalidate dashboard stats query', 'query_invalidation', {
            userId,
            error: error.message
          });
        }
        ...
        logApi('info', 'Data refresh completed successfully', 'data_refresh', { userId });
        ...
        logApi('error', 'Error during post-upload processing', 'data_refresh', {
          userId,
          error: error.message
        });
        setIsProcessing(false);
